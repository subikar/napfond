...
<select onchange="javascript:jQuery('#filter_loading_image').show();load_fonts_by_cat(this.value)">
<?php
foreach($collection as $collItem)
 ...
 if($collItem['category_id'] == 1)
  continue;
?>
<option value="<?php echo $collItem['category_id']?>" <?php if($get_cat_id == $collItem['category_id']) echo 'selected';?>><?php echo $collItem['name']?></option>
<?php  
 }
?>
</select>
...
<div class="sright" style="max-width:300px">        
        <?php 
		if($firstCategory == 65){
		foreach ($defaultFonts as $font):?>
           <?php /*?> <option value="<?php echo $font['value']; ?>" <?php echo Mage::getStoreConfig('gomage_designer/text/font') == $font['value'] ? 'selected="selected"' : ''?>><?php echo $font['label']; ?></option><?php */?>
			...
			<div class="image-ppc-font bhishoom_font_selector" onclick="parent.jQuery('#font-selector').val('<?php echo $font['value']; ?>').trigger('change'); parent.objCallDirect.customBhishoom_observeFontChange();parent.jQuery.fancybox.close();" style="font-size:14px; font-family:<?php echo $font['label']?>;cursor:pointer;padding: 0px !important;width:88px; height:35px;"><?php echo $font['label']; ?></div>
...
<div class="image-ppc-font bhishoom_font_selector" onclick="parent.jQuery('#font-selector').val('<?php echo $fontValue; ?>'); parent.objCallDirect.customBhishoom_observeFontChange();parent.jQuery.fancybox.close();" id="<?php echo $fontNameIdToDisplay?>" style="font-size:14px;cursor:pointer;padding: 5px !important;display:block !important;width:88px; height:35px;"><span class="demoTest"><?php echo $fontNameToDisplay; ?></span></div>				
