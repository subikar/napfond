    ...
    public function downloadAllAction()
    {
        ...
        if ($designId) {
			...
			if($gravitaSessionId == '')
                ...
                $archive->addFile(
                    Mage::getModel('gomage_designer/design_config')->getMediaPath($_image->getSkinImage()),
                    ltrim($_image->getSkinImage(), '/')
                );
                if ($_image->getLayer()) {
                    $archive->addFile(
			...
			$_imagesCollection = Mage::getModel('gomage_designer/design')->_initUploadedImagesCollection($gravitaSessionId);
			
			foreach ($_imagesCollection as $_image):
			 
				if(is_file(Mage::getSingleton('gomage_designer/uploadedImage_config')->getBaseMediaPath().$_image->getImage()))	
			    $archive->addFile(
                    Mage::getSingleton('gomage_designer/uploadedImage_config')->getBaseMediaPath().$_image->getImage(),
                    ltrim($_image->getImage(), '/')
                );
