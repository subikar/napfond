...
class Mage_Adminhtml_Block_Sales_Order_Shipment_View extends Mage_Adminhtml_Block_Widget_Form_Container
{
        ...
        if ($this->getShipment()->getId()) {
			...
			if(count($pickupDetailsArr)>0){
				$this->_addButton('cancel_pickup', array(
					...
					'class'     => 'cancel',
					'onclick'   => 'setLocation(\''.$this->getCancelPickupUrl().'\')'
					)
				);
			}
			else{
				
				$this->_addButton('request_pickup', array(
					'label'     => Mage::helper('sales')->__('Request Pickup'),
					'class'     => 'save',
					'onclick'   => 'setLocationBhishoomTSh(\''.$this->getRequestPickupUrl().'\')'
					)
				);
    ...
    public function getRequestPickupUrl()
    {
        return $this->getUrl('*/sales_order_shipment/requestpickup', array('shipment_id'  => $this->getShipment()->getId()));
    }

    public function getCancelPickupUrl()
    {
        return $this->getUrl('*/sales_order_shipment/cancelpickup', array('shipment_id'  => $this->getShipment()->getId()));
    }
