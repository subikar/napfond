		...
		$sub_subCatid = $sub_subCatids->getId();
          ...
          if($_sub_category->getIsActive()) {
			  ...
			  $subname = $_sub_category->getName();
			   ...
			   $menuStr = $menuStr.'<li><a href="'.$this->baseUrl.'designer?id='.$blankPrdId.'&cat_id='.$_sub_category->getId().'">'.$subname.'</a>
</li>';
...
foreach($subcats as $subCatids)
{
  ...
  if($_category->getIsActive()) {
	  $url =  $_category->getURL();
	  $name = $_category->getName();
    $menuStr = $menuStr.'<li><a href="'.$url.'">'.$name.'<span><i class="fa fa-plus"></i></span></a>
</li>';
