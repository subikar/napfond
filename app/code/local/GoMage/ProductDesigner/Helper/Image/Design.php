...
class GoMage_ProductDesigner_Helper_Image_Design extends GoMage_ProductDesigner_Helper_Image_Abstract
{
    ...
    {
        $path = Mage::getSingleton('gomage_designer/design_config')->getBaseMediaUrl() . '/' . 'cache';
        if ($this->_width || $this->_height) {
            $path .= "/{$this->_width}_{$this->_height}";
        }

        return $path . '/' . ltrim($this->_filename, '/');
    }
    ...
    {
        $dir = Mage::getSingleton('gomage_designer/design_config')->getBaseMediaPath() . '/' . 'cache';
        if ($this->_width || $this->_height) {
            $dir .= '/' . $this->_width . '_' . $this->_height;
        }
        return $dir;
    }
}
