    ...
    implements Mage_Shipping_Model_Carrier_Interface
{
    ...
    public function __construct()
    {
        ...
        $this->_shipServiceWsdl = $wsdlBasePath . 'ShipService_v17.wsdl';
        $this->_rateServiceWsdl = $wsdlBasePath . 'RateService_v10.wsdl';
        $this->_trackServiceWsdl = $wsdlBasePath . 'TrackService_v10.wsdl';
        //$this->_pickupServiceWsdl = $wsdlBasePath . 'PickupService_v11.wsdl';
    }
    ...
    /**
     * Create pickup soap client
     *
     * @return SoapClient
     */
    /*protected function _createPickupSoapClient()
    {
        return $this->_createSoapClient($this->_pickupServiceWsdl, 1);
    }*/
    ...
    protected function _formRateRequest($purpose)
    {
		...
		$setFedexCommercialInvoicePurpose_Key = Mage::registry($key); 
...
if($setFedexCommercialInvoicePurpose_Key!='')
 $CommercialInvoice_Sold = $setFedexCommercialInvoicePurpose_Key;
else{
	...
	$cartSubTotal = $readConnection->fetchOne("SELECT subtotal FROM sales_flat_quote where entity_id='".$quoteId."'");
	...
	if($subTotalWithDiscount > 0)	
		$cartSubTotal = $subTotalWithDiscount;
		
		 
		$getPaymentData = Mage::helper('checkout/cart')->getQuote()->getData();
		//echo '<pre>';
		//print_r($getPaymentData );
			...
			if($cartSubTotal <= 5000 && $getPaymentData['cod_fee'] == 0)
				$CommercialInvoice_Sold = 'NOT_SOLD';
			else
				$CommercialInvoice_Sold = 'SOLD';
            ...
            'RequestedShipment' => array(
                ...
                'CustomsClearanceDetail' => array(
					...
					'CommercialInvoice' => array(
					   'Purpose' => $CommercialInvoice_Sold
					)
                ),
    ...
    protected function _prepareRateResponse($response)
    {
        ...
        if (empty($priceArr)) {
			 /*
                ...
                $rate->setCarrierTitle('Federal Express');
     ...
     * @return mixed
     */
    public function getTracking($trackings,$getShipMentId=null)
    {
        ...
        foreach($trackings as $tracking){
			...
			if(is_null($getShipMentId)){
			$shipMentId=Mage::getResourceModel('sales/order_shipment_track_collection')->addAttributeToSelect('*')->addAttributeToFilter('track_number',$tracking);
			$getShipMentId = $shipMentId->getData();
			$getShipMentId = $getShipMentId[0]['parent_id'];
			}
            $this->_getXMLTracking($tracking,$getShipMentId);
        }
     ...
     * @return void
     */
    protected function _getXMLTracking($tracking,$shipMentId)
    {
		
		$deliveryEmailHtml = Mage::helper('function/more')->getDeliveryEmailTemplate($shipMentId,true,false);
		...
		$trackRequest = array(
            ...
            'NotificationDetail' => array(
											'PersonalMessage' => $deliveryEmailHtml,
											'Recipients' => array(
												'0' => array(
													'EMailNotificationRecipientType' => 'RECIPIENT',
													'EMailAddress' => 'mukeshgyanani94@gmail.com',
        ...
        $this->_parseTrackingResponse($tracking, $response, $shipMentId);
    }

	
	/*function getDeliveryEmailTemplate($shipMentId)
	{
    ...
    protected function _parseTrackingResponse($trackingValue, $response, $shipMentId)
    {
	...
	//$writeConnection = Mage::helper('function/more')->getDbWriteObject();;	
        ...
        if (is_object($response)) {	
			...
			elseif (isset($response->CompletedTrackDetails->TrackDetails)) {
                ...
                $resultArray['status'] = (string)$trackInfo->StatusDetail->Description;
                $resultArray['service'] = (string)$trackInfo->Service->Description;
                $timestamp = isset($trackInfo->ShipTimestamp) ?
                    $trackInfo->ShipTimestamp : $trackInfo->ActualDeliveryTimestamp;
                ...
                $deliveryLocation = isset($trackInfo->DestinationAddress) ?
                    $trackInfo->DestinationAddress : $trackInfo->ActualDeliveryAddress;
                ...
                if (isset($trackInfo->Events)) {
                    ...
                    foreach ($events as $event) {
                        ...
                        $tempArray['activityCode'] = (string)$event->EventType;
						
						if($tempArray['activityCode'] == 'DL'){
							$isDelivered=true;
						}
						if($tempArray['activityCode'] == 'OD'){
        ...
        if (isset($resultArray)) {
			...
			$shipMent_Track_collection=Mage::getResourceModel('sales/order_shipment_track_collection')->addAttributeToSelect('*')->addAttributeToFilter('track_number',$trackingValue);
			
			foreach($shipMent_Track_collection as $shipMent_Track_collection_val){
				
				$getLatestTrackInfo = $shipMent_Track_collection_val->getLatestTrackInfo();
				
				
				
				if($isOutfordelivery == true && !strpos($getLatestTrackInfo,'On FedEx vehicle for delivery')){
					Mage::helper('function/sms')->sendOutForDeliverySms($shipMentId);
				}
				if($isDelivered == true){					
					Mage::helper('function/more')->getDeliveryEmailTemplate($shipMentId,false,true);
					Mage::helper('function/sms')->sendDeliverySms($shipMentId);
					
					
					
					//$deliveryEmailHtml = $this->getDeliveryEmailTemplate($shipMentId,false,true);
				}
				
				$shipMent_Track_collection_val->setLatestTrackInfo(serialize($resultArray['progressdetail']));
				$shipMent_Track_collection_val->save();			
			}
			
			//$shipMent_Track_collection->setLatestTrackInfo(serialize($resultArray['progressdetail']))->save();
			
			//$writeConnection->query("update sales_flat_shipment_track set latest_track_info='".serialize($resultArray['progressdetail'])."' where track_number='".$trackingValue."'");		
    ...
    protected function _getAuthDetailsShip()
    {
        return array(
            'WebAuthenticationDetail' => array(
            ...
            'TransactionDetail' => array(
                'CustomerTransactionId' => '*** Express Domestic Shipping Request v17 using PHP ***'
            ),
            'Version' => array(
                'ServiceId'     => 'ship',
                'Major'         => '17',
    ...
    protected function _formShipmentRequest(Varien_Object $request)
    {
		 ...
		 $customsValue = $request->getOrderShipment()->getOrder()->getGrandTotal();
		 $order = $request->getOrderShipment()->getOrder();
		 
		 //$feeAmount = $order->getFeeAmount();
		 
		 $orderItemPriceArray = array();
		 $customsValue = 0;
				$orderItems = $order->getItemsCollection();
				//$applyFeeAmount = false;
				foreach ($orderItems as $item){
					...
					if($item->cod_applicable=='y')
					$orderItemPriceArray[$item->item_id] = $item->item_shipping_price + Mage::getStoreConfig('payment/phoenix_cashondelivery/inlandcosts');
					else
					$orderItemPriceArray[$item->item_id] = $item->item_shipping_price;
					/*
					if($applyFeeAmount == false)
					{
						$orderItemPriceArray[$item->item_id] = $orderItemPriceArray[$item->item_id] + $feeAmount;
						if($orderItemPriceArray[$item->item_id] >= 0)
						$applyFeeAmount = true;
						else	
		 ...
		 $newtext = wordwrap(trim($request->getRecipientAddressStreet1()), 35, "\n", false);
		 $recepientAddressArr = explode("\n",$newtext);
		 
		 $companyName = '';
		 $streetAddress1 = '';
		 $streetAddress2 = '';
		 
		 if(count($recepientAddressArr) > 2)
		 {
			 $companyName = $recepientAddressArr[0];
			 $streetAddress1 = $recepientAddressArr[1];
			 $streetAddress2 = $recepientAddressArr[2];	 
		 }
		 elseif(count($recepientAddressArr) == 2)
		 {
			 $companyName = $request->getRecipientContactCompanyName();
			 $streetAddress1 = $recepientAddressArr[0];
			 $streetAddress2 = $recepientAddressArr[1];			 
		 }		 
		 elseif(count($recepientAddressArr) == 1)
		 {
			 $companyName = '';
			 $streetAddress1 = $recepientAddressArr[0];
			 $streetAddress2 = '';			 
        ...
        foreach ($packageItems as $itemShipment) {
				...
				$customsValue = $customsValue + ($item->getPrice() * $item->getQty());
				
				$customsValue = $customsValue + $orderItemPriceArray[$item->getData('order_item_id')];
				
               // $itemsDesc[]    = $item->getName();
			   $tempName = '';
			   
			   if(strpos(strtolower($item->getName()),'mobile skin') > 0)
			   $tempName    = 'Mobile Skin';
			   else if(strpos(strtolower($item->getName()),'mobile case') > 0)
			   $tempName    = 'Mobile Case';
			   else if(strpos(strtolower($item->getName()),'laptop skin') > 0)
			   $tempName    = 'Laptop Skin';
			   else if(strpos(strtolower($item->getName()),'tablet skin') > 0)
			   $tempName    = 'Tablet Skin';
			   else if(strpos(strtolower($item->getName()),'t-shirt') > 0)
			   $tempName    = 'T-shirt';
			   else if(strpos(strtolower($item->getName()),'custom skin') > 0)
			   $tempName    = 'Custom Skin';
			   else if(strpos(strtolower($item->getName()),'custom case') > 0)
			   $tempName    = 'Custom Case';
				
			   if(!in_array($tempName,$itemsDesc) > 0)	
			   $itemsDesc[] = $tempName;
        ...
        foreach ($productCollection as $product) {
            $countriesOfManufacture[] = 'IN';
        }		 
		 
		 
		 
		 if($request->getShippingMethod() == 'STANDARD_OVERNIGHT'){
			 $Purpose = 'SOLD';	
			 
			/* 
			$SpecialServicesRequested = array(
                        'SpecialServiceTypes' => array('COD','SIGNATURE_OPTION'),
								'CodDetail' => array(
					'CodCollectionAmount' => array(
								'Currency' => $request->getBaseCurrencyCode(), 
								'Amount' => Mage::getStoreConfig('payment/phoenix_cashondelivery/inlandcosts')
					),
					...
					/*
			$SpecialServicesRequested = array(array(
        ...
        $requestClient = array(
            'RequestedShipment' => array(
                ...
                'LabelSpecification' =>array(
                    ...
                    'LabelStockType' => 'PAPER_8.5X11_TOP_HALF_LABEL',//PAPER_LETTER
                ),
            ...
            $requestClient['RequestedShipment']['CustomsClearanceDetail'] =
                array(
					...
					'CommercialInvoice' => array(
					   ...
					   'CustomerReferences' => array(
							'CustomerReferenceType' => 'CUSTOMER_REFERENCE',
							'Value' => $request->getOrderShipment()->getOrder()->getIncrementId()
						)
					)
					,
                    'DutiesPayment' => array(
						'PaymentType' => 'SENDER', // $paymentType, valid values RECIPIENT, SENDER and THIRD_PARTY
						'Payor' => array(
							'ResponsibleParty' => array(
								'AccountNumber' => $this->getConfigData('account'),
								'Contact' => null,
		...
		//echo $customsValue;exit;
		if($payment_method_code == 'phoenix_cashondelivery')
		$requestClient['RequestedShipment']['RequestedPackageLineItems']['CustomerReferences'] = array(array(
			...
			'Value' => $request->getOrderShipment()->getOrder()->getIncrementId()),array(
			'CustomerReferenceType' => 'P_O_NUMBER', // valid values CUSTOMER_REFERENCE, INVOICE_NUMBER, P_O_NUMBER and SHIPMENT_INTEGRITY
			'Value' => 'B2C'),array(
			'CustomerReferenceType' => 'DEPARTMENT_NUMBER', // valid values CUSTOMER_REFERENCE, INVOICE_NUMBER, P_O_NUMBER and SHIPMENT_INTEGRITY
			'Value' => 'BILL DUTIES: SENDER'),array(
			'CustomerReferenceType' => 'INVOICE_NUMBER', // valid values CUSTOMER_REFERENCE, INVOICE_NUMBER, P_O_NUMBER and SHIPMENT_INTEGRITY
			'Value' => 'Amount to collect: '.$request->getBaseCurrencyCode().' '.$customsValue),
		);
		else
		$requestClient['RequestedShipment']['RequestedPackageLineItems']['CustomerReferences'] = array(array(
			'CustomerReferenceType' => 'CUSTOMER_REFERENCE', // valid values CUSTOMER_REFERENCE, INVOICE_NUMBER, P_O_NUMBER and SHIPMENT_INTEGRITY
			...
			'Value' => 'B2C'),array(
			'CustomerReferenceType' => 'DEPARTMENT_NUMBER', // valid values CUSTOMER_REFERENCE, INVOICE_NUMBER, P_O_NUMBER and SHIPMENT_INTEGRITY
			'Value' => 'BILL DUTIES: SENDER'),
		);
			

        return $this->_getAuthDetailsShip() + $requestClient;
    }
    ...
    {		
        ...
        if ($response->HighestSeverity != 'FAILURE' && $response->HighestSeverity != 'ERROR') {
			...
			if($response->CompletedShipmentDetail->AssociatedShipments->Label->Parts->Image!='')
			{
				$shippingLabelContent_COD_Return = $response->CompletedShipmentDetail->AssociatedShipments->Label->Parts->Image;				
			}
			...
			if($shippingLabelContent_COD_Return!=''){
				
				$result->setCodReturnShippingLabelContent($shippingLabelContent_COD_Return);
			}
