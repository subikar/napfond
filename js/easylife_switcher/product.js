    ...
    transformDropdown: function(selectid){
        ...
        if (switchConfig && typeof(switchConfig) == "object") {
            ...
            $(selectid).childElements().each(function(elem, index){
                ...
                $(newId).insert('<label class="switcher-label' + labelClass + ' " id="' + $(selectid).id + '_' + optVal + '" value="' + optVal + '"><span>'+optText+'</span></label>');
    ...
    getOptionText: function(elem, value, config){
        ...
        switch (config.type) {
            ...
            case 'colors':
                ...
                if (color) {
                    text = '<span class="switcher-hexacode tshirt_class_'+text.split(' ').join('_').toLowerCase()+'" style="background-color:' + color +'"></span><span>'+ text + '</span>'
                }
                break;
            default:
