    ...
    */
    ...
    public function getStockOptions()
    {
        ...
        foreach ($simpleProducts as $product) {
            ...
            $stock[$productId] = $product->getStockItem()->getIsInStock();
			
			////////////////////////////////////////Custom code for fetching inventory for tshirt////////////////////////////////////
			$simpleProductId = $productId;
			$parentIds = Mage::getResourceSingleton('catalog/product_type_configurable')
                  ->getParentIdsByChild($simpleProductId);
			if($parentIds[0] > 0){	  
			$productConfigurable = Mage::getModel('catalog/product')->load($parentIds[0]);
		    if($productConfigurable->getStyleCode() != '')
			{
							$getInventory = $read->query("select * from  a_create_tshirt_inventory  where designId = '".$productConfigurable->getStyleCode()."'");
							$inventory_details = $getInventory->fetchAll();
							$incentoRy = unserialize(base64_decode($inventory_details[0]['inVentory']));							
							$tshirtStockOnApi = $incentoRy[ucwords(strtolower(trim($product->getAttributeText('color'))))][strtoupper(strtolower(trim($product->getAttributeText('size'))))][0];
							if($tshirtStockOnApi > 0)
								$stock[$productId] =  true;
							else
