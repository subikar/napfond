...
class Mage_CatalogInventory_Model_Stock_Item extends Mage_Core_Model_Abstract
{
    ...
    public function checkQuoteItemQty($qty, $summaryQty, $origQty = 0)
    {
		...
		$checkInventoryByCategoryVar = Mage::helper('function/more')->checkInventoryByCategory($_customOptions['info_buyRequest']['bhishoom_product_fromCat']);
		//echo $checkInventoryByCategoryVar;
		if($checkInventoryByCategoryVar != -1 && $checkInventoryByCategoryVar !==false){
		
				if($checkInventoryByCategoryVar == 0 && $$checkInventoryByCategoryVar != ''){
				...
				elseif(($qty > $checkInventoryByCategoryVar) && ($checkInventoryByCategoryVar > 0)){
