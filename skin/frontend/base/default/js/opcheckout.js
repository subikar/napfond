...
Checkout.prototype = {
    initialize: function(accordion, urls){
        ...
        this.accordion.sections.each(function(section) {
            Event.observe($(section).down('.mycustomEle'), 'click', this._onSectionClick.bindAsEventListener(this));
        }.bind(this));
    ...
    gotoSection: function (section, reloadProgressBlock) {
		...
		jQuery( '#opc-' + section ).prev( "#checkoutSteps .step-title" ).addClass("thisActive");
	
	 if(section == 'billing')
		jQuery('#opc-block-progress-id').addClass('displayNone');
        if(!reloadProgressBlock) {
...
Billing.prototype = {
    initialize: function(form, addressUrl, saveUrl){
        ...
        /*var curr_id = jQuery('.active').attr('id');
        if(curr_id == 'opc-billing')
        {   
            var hj = jQuery('#checkoutSteps > #opc-billing > #checkout-step-billing').innerHeight();
            var hj = hj + 250;
            jQuery('#checkoutSteps').css({'height': hj});
        }*/
    },
...
//            }
    ...
    nextStep: function(transport){
		...
		jQuery("#opc-block-progress-id").removeClass("displayNone");
...
Payment.prototype = {
    ...
    init : function () {
        ...
        for (var i=0; i<elements.length; i++) {
            ...
            }else if (elements[i].name=='payment[drop_category]') {
                if (elements[i].value!='') {
                    drop_category = elements[i].value;
                }
            } else {
...
Review.prototype = {
    initialize: function(saveUrl, successUrl, agreementsForm){
        ...
        this.onComplete = this.resetLoadWaiting.bindAsEventListener(this);
        var curr_id = jQuery('.active').attr('id');
        if(curr_id == 'opc-review')
        {   
            var hj = jQuery('#checkoutSteps > #opc-review > #checkout-step-review').innerHeight();
            var hj = hj + 110;
            jQuery('#checkoutSteps').css({'height': hj});
        }
    },
