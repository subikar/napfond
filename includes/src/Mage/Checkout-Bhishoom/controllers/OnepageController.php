...
class Mage_Checkout_OnepageController extends Mage_Checkout_Controller_Action
{
   ...
   /* public function saveBillingAction()
      ...
      if ($this->getRequest()->isPost()) {
	   ...
	   if($total > $minimum)
		...
		if($data['use_for_shipping'] == 1){
         	$result = $this->saveShippingMethodAction($method);
		}
		
		 $quote_id = Mage::getSingleton('checkout/cart')->getQuote()->getEntity_id();
		 $write = Mage::getSingleton( 'core/resource' )->getConnection( 'core_write' );
		 $write->query("update sales_flat_quote_payment set method='checkmo' where quote_id='".$quote_id."'" );
       ...
       if (!isset($result['error'])) {
        if ($this->getOnepage()->getQuote()->isVirtual()) {
			...
			$result['update_section'] = array(
			 ...
			 // $this->saveShippingMethodAction($method);
			  $this->loadLayout('checkout_onepage_review');
				 $result['goto_section'] = 'review';
				 $result['update_section'] = array(
				  'name' => 'review',
     ...
     */
    public function saveShippingMethodAction($shipping_method_action = '')
    { 
			if ($this->_expireAjax()) {
				return;
			}         
		 ...
		 */
		    /////////////////////////Custom Code By Lokesh Yadav/////////////////////////////
            $data = $shipping_method_action;
            $result = $this->getOnepage()->saveShippingMethod($data);
            // $result will contain error data if shipping method is empty
            /*if (!$result) {
                Mage::dispatchEvent(
	...
	public function saveReviewAction()
    {
		...
		if ($this->getRequest()->isPost()) {
            $data = $this->getRequest()->getPost('review', '');
            $result = $this->getOnepage()->saveReviewMethod($data);
            // $result will contain error data if shipping method is empty
            if (!$result) {
...
if ($this->_expireAjax()) {
    ...
    public function saveGraFinalOrderAction()
    {
...
if ($this->_expireAjax()) {
 ...
 if (!$this->_validateFormKey()) {
        ...
        try {
			...
			$this->getOnepage()->saveShippingMethod('flatrate_flatrate');
