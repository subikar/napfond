 ...
 */
class GoMage_ProductDesigner_Model_Mysql4_Fontswithcat_Category_Collection
    extends Mage_Core_Model_Mysql4_Collection_Abstract
{
    ...
    public function addClipartCountFilter()
    {
        ...
        $countSelect = $this->getConnection()->select()
            ->from(array('fontswithcat_category' => $this->getMainTable()), null)
            ->joinLeft(
                array('fonts' => $this->getTable('gomage_designer/font')),
                'fontswithcat_category.category_id=fonts.category_id',
                array('COUNT(DISTINCT fonts.clipart_id)'))
            ->where('fontswithcat_category.category_id = main_table.category_id')
            ->orWhere('fontswithcat_category.path LIKE ?', $concatExpr);
