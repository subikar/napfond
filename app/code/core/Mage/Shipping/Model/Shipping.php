...
class Mage_Shipping_Model_Shipping
{
    ...
    public function requestToShipment(Mage_Sales_Model_Order_Shipment $orderShipment)
    {
		...
		///////////////////////////////////////////Code to identify the Tshirt Package////////////////////////////////////////////////////
		/*$orderItems = $order->getAllVisibleItems();
		$pkgType = '';
		foreach ($orderItems as $items){
			if($items->getData("prdType")=='t-shirt'){
			
			if($pkgType != '' && $pkgType != 't-shirt'){
				$pkgType = 'error';
			}else
				$pkgType = 't-shirt';
			}else{
		...
		if($pkgType == 'error'){
			Mage::throwException('Invalid pacakage: You can not mix tshirt and other product in one package');
		}*/
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	/////////////////////////////////////Request Shipment For Tshirt Order//////////////////////////////////////////////////////
	
	
    public function requestToShipmentTshirt(Mage_Sales_Model_Order_Shipment $orderShipment)
    {
        $admin = Mage::getSingleton('admin/session')->getUser();
        $order = $orderShipment->getOrder();
        ...
        $request->setShipperContactPersonName(Mage::getStoreConfig('shipping/origin_tshirt/tshirt_contact_person'));
        //$request->setShipperContactPersonFirstName($admin->getFirstname());
        //$request->setShipperContactPersonLastName($admin->getLastname());
        $request->setShipperContactCompanyName(Mage::getStoreConfig('shipping/origin_tshirt/tshirt_contact_company'));
        $request->setShipperContactPhoneNumber(Mage::getStoreConfig('shipping/origin_tshirt/tshirt_contact_phone'));
        $request->setShipperEmail(Mage::getStoreConfig('shipping/origin_tshirt/tshirt_contact_email'));
        $request->setShipperAddressStreet(trim(Mage::getStoreConfig('shipping/origin_tshirt/tshirt_street_line1') . ' ' . Mage::getStoreConfig('shipping/origin_tshirt/tshirt_street_line2')));
        $request->setShipperAddressStreet1(Mage::getStoreConfig('shipping/origin_tshirt/tshirt_street_line1'));
        $request->setShipperAddressStreet2(Mage::getStoreConfig('shipping/origin_tshirt/tshirt_street_line2'));
        $request->setShipperAddressCity(Mage::getStoreConfig('shipping/origin_tshirt/tshirt_city', $shipmentStoreId));
        ...
        $request->setShipperAddressStateOrProvinceCode($shipperRegionCode);
        $request->setShipperAddressPostalCode(Mage::getStoreConfig('shipping/origin_tshirt/tshirt_postcode', $shipmentStoreId));
        $request->setShipperAddressCountryCode(Mage::getStoreConfig('shipping/origin_tshirt/tshirt_country_id', $shipmentStoreId));
