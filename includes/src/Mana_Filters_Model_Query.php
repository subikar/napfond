...
class Mana_Filters_Model_Query extends Varien_Object
{
    ...
    protected function _init() {
		...
		//print_r($arrParams);
		//exit; 
		//$this->_productCollection = $this->_productCollection->addAttributeToFilter('name', array('in' => array('black evil skull','boombox')));
		//echo $this->_productCollection->getSize();
		if (array_key_exists('name', $arrParams)) { 
		foreach($arrParams as $arrParamsKey=>$arrParamsVal){
			if($arrParamsKey == 'name')
			{
				$this->_productCollection = $this->_productCollection->addAttributeToFilter($arrParamsKey, array('in' => $arrParamsVal));
