...
class Unirgy_Giftcert_Helper_Data extends Mage_Core_Helper_Data
{
    ...
    */
    public function getGiftcertOptionVars()
    {
        return array(
            ...
            'recipient_phone' => $this->__("Recipient's phone number"),
    ...
    */
    ...
    {   
        ...
        if($data->getIssenderemail() == 1)
        {  
        	
        	$gc = $data->getGc();  
            //$d = $gc->getExpireAt();
            //$nd = explode("-", $d);    
			$new_date = $this->formatDate($gc->getExpireAt(), 'long');
        	           
        	 /*$template = Mage::getStoreConfig('ugiftcert/email/template_sender', $store);
            ...
            $emailTemplate = Mage::getModel('core/email_template')->loadDefault('ugiftcert_email_template_sender');	 	
		 	$emailTemplateVariables = array();
			$emailTemplateVariables['sender_name'] = $data->getSenderName();
			$emailTemplateVariables['sender_firstname'] = $data->getSenderFirstname();
			$emailTemplateVariables['receiver_name'] = ucwords($gc->getRecipientName());	
			$emailTemplateVariables['amount'] = round($gc->getAmount());
			$emailTemplateVariables['validity'] = $new_date;	
			$emailTemplateVariables['message'] = $gc->getRecipientMessage();
			$emailTemplateVariables['code'] = $gc->getCertNumber();
			$emailTemplate->setSenderName('Bhishoom');
            $emailTemplate->setSenderEmail('info@bhishoom.com');
            $emailTemplate->setType('html');
            $emailTemplate->setTemplateSubject('Detail of your gift voucher purchased at Bhishoom.com');
            $emailTemplate->send($data->getSenderEmail(), $data->getSenderName(), $emailTemplateVariables);
        }
        else
        {        	
            $template = Mage::getStoreConfig($self ? 'ugiftcert/email/template_self' : 'ugiftcert/email/template', $store);
            
			$r_name = explode(' ',$data->getName());
			$r_name = $r_name[0];
    ...
    */
    public function sendOrderItemEmail($item, $giftcerts = null)
    {
        ...
        foreach ($giftcerts as $gc) {
            //print_r($gc);die;
            if($gc->getCustomerEmail() != '')
            {
                $sender_email = $gc->getCustomerEmail();
                $sender_name = $gc->getSenderName();
            }            
            if (is_null($self)) {
                $self = !$gc->getRecipientName();
        ...
        $this->sendEmail(array(
            ...
            'sender_name' => $order->getCustomerName() ? $order->getCustomerName() : $order->getBillingAddress()->getName(),          
            'sender_firstname' => $order->getCustomerFirstname() ? $order->getCustomerFirstname() : $order->getBillingAddress()->getFirstname(),
            'sender_email' => $order->getCustomerEmail() ? $order->getCustomerEmail() : $order->getBillingAddress()->getEmail(),    
            'issenderemail' => 1,
            'gc' => $gc,
            'gc_numbers' => $gcs,
        ));
