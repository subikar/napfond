...
public function updateOrderItemInfoForShipping(Varien_Event_Observer $observer)
	{
		...
		foreach($itemDetailsAll as $itemDetailsOrder)
		{
			...
			$product_options_in_tbl = unserialize($itemDetailsOrder['product_options']);
			
			$checkInventoryByCategoryVar = Mage::helper('function/more')->checkInventoryByCategory($product_options_in_tbl['info_buyRequest']['bhishoom_product_fromCat']);
			//echo $checkInventoryByCategoryVar;
			if($checkInventoryByCategoryVar != -1 && $checkInventoryByCategoryVar !==false){
			
			 $category_id_value = $product_options_in_tbl['info_buyRequest']['bhishoom_product_fromCat'];
			
			 $updated_cat_stock_value = $checkInventoryByCategoryVar - $itemDetailsOrder["qty_ordered"];	
			
			 $query = "update catalog_category_entity_int set value='".$updated_cat_stock_value."' where entity_id = '".$category_id_value."' and attribute_id=197";
			 $write->query($query);				
			 $query = "update catalog_category_flat_store_1 set category_num_stock_item='".$updated_cat_stock_value."' where entity_id = '".$category_id_value."'";
			 $write->query($query);				
...
public function salesOrderItemCancel(Varien_Event_Observer $observer)
	{
		$itemDetailsOrder = $observer->getEvent()->getItem();
		...
		$read = Mage::helper('function/more')->getDbReadObject();		
			...
			if($itemDetailsOrder->getData("product_type")=='configurable')
				continue;
						
			$product_options_in_tbl = unserialize($itemDetailsOrder->getData('product_options'));
			...
			if($checkInventoryByCategoryVar != -1 && $checkInventoryByCategoryVar !==false){
				 ...
				 $updated_cat_stock_value = $checkInventoryByCategoryVar + $itemDetailsOrder->getData("qty_ordered");	
