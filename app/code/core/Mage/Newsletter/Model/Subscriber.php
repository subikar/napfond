...
class Mage_Newsletter_Model_Subscriber extends Mage_Core_Model_Abstract
{
     ...
     * @return int
     */
    public function subscribe($email, $postname = NULL)
    {
        ...
        try {
			...
			if($subscriber)
			{
				...
				$search = strstr($postname, " ", true);
				if($search)
				{
					$name = explode(" ", $postname);
					$fname = $name[0];
					$lname = $name[1];	
					echo "INSERT INTO `customer_entity_varchar` (`entity_type_id`, `attribute_id`, `entity_id`, `value`) VALUES (1, 5, '".$customer_id."', '".$fname."')";
					echo '<br>';
					echo "INSERT INTO `customer_entity_varchar` (`entity_type_id`, `attribute_id`, `entity_id`, `value`) VALUES (1, 7, '".$customer_id."', '".$lname."')";
					$write->query("INSERT INTO `customer_entity_varchar` (`entity_type_id`, `attribute_id`, `entity_id`, `value`) VALUES (1, 5, '".$customer_id."', '".$fname."')");		
					$write->query("INSERT INTO `customer_entity_varchar` (`entity_type_id`, `attribute_id`, `entity_id`, `value`) VALUES (1, 7, '".$customer_id."', '".$lname."')");		
				}
				else
				{
					echo "INSERT INTO `customer_entity_varchar` (`entity_type_id`, `attribute_id`, `entity_id`, `value`) VALUES (1, 5, '".$customer_id."', '".$postname."')";
					$write->query("INSERT INTO `customer_entity_varchar` (`entity_type_id`, `attribute_id`, `entity_id`, `value`) VALUES (1, 5, '".$customer_id."', '".$postname."')");		
				}				
			}
			print_r($subscriber);die;
		  // print_r($this);die;*/
		
		  if(!Mage::app()->getRequest()->getParam('fromFrontRegisterForm')){
		    if ($isConfirmNeed === true
                && $isOwnSubscribes === false
            ) {
    ...
    {
		$fromFrontRegisterForm = Mage::app()->getRequest()->getParam('fromFrontRegisterForm'); 
		
		$fromCheckoutPage = Mage::app()->getRequest()->getParam('payment'); 
		$fromCheckoutPage = $fromCheckoutPage['method'];
		
	    //echo $this->getRequest()->getParams('fromFrontRegisterForm');
		//echo 'helllooo 12333';
		//exit;
       ...
       if ($customer->isConfirmationRequired() && ($customer->getConfirmation() != $customer->getPassword())) {
        ...
        if (is_null($sendSubscription) xor $sendSubscription) {
            if ($this->getIsStatusChanged() && $status == self::STATUS_UNSUBSCRIBED) {
				...
				if(!$fromFrontRegisterForm && !$fromCheckoutPage)
                $this->sendConfirmationSuccessEmail();
            }
        }
        return $this;
