 ...
 */
class GoMage_ProductDesigner_Block_Adminhtml_Fontswithcats_Edit_Gallery
    extends Mage_Adminhtml_Block_Catalog_Product_Helper_Form_Gallery_Content
{
    public function __construct()
    {
        parent::__construct();
        $this->setTemplate('gomage/productdesigner/fontswithcats/edit/gallery.phtml');
    }
    ...
    protected function _prepareLayout()
    {
        ...
        $this->getUploader()->getConfig()
            ...
            ->setFilters(array(
                'images' => array(
                    'label' => Mage::helper('adminhtml')->__('Images (.gif, .jpg, .png, .svg, .ttf, .otf)'),
                    'files' => array('*.gif', '*.jpg','*.jpeg', '*.png', '*.svg', '*.ttf', '*.otf')
                )
            ));
    ...
    public function getImagesJson()
    {
        ...
        $galleryConfig = Mage::getSingleton('gomage_designer/fontswithcat_gallery_config');
        $mediaUrl = $galleryConfig->getBaseMediaUrl();

        if($category) {
            foreach($category->getClipartsCollection() as $image) {
                $imageData = $image->getData();
                $imageUrl = $imageData['font'];
                $valueId = $imageData['font_id'];
