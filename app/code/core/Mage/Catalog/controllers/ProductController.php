...
class Mage_Catalog_ProductController extends Mage_Core_Controller_Front_Action
{
    ...
    public function viewAction()
    {
		...
		if($style_code!=''){
			...
			//echo $product_entity_id = $configProduct['entity_id'];
			//echo '<br/>';
			//echo $style_code = $configProduct['style_code'];
			//echo '<br/>';
			//echo $configProduct['type_id'];
			//exit;			
			
			$read = Mage::helper('function/more')->getDbReadObject();
			$write = Mage::helper('function/more')->getDbWriteObject();
			
					if ($configProduct['type_id'] == 'configurable') {
						$product_entity_id = $configProduct['entity_id'];
						// Get any super_attribute settings we need
							$getInventory = $read->query("select * from  a_create_tshirt_inventory  where designId = '".$style_code."'");
							$inventory_details = $getInventory->fetchAll();
							$incentoRy = unserialize(base64_decode($inventory_details[0]['inVentory']));						
							
								$childIds = Mage::getModel('catalog/product_type_configurable')->getChildrenIds($product_entity_id);
								foreach($childIds[0] as $key=>$val)
								{
									
								   $associatedProduct = Mage::getModel('catalog/product')->load($val);
								   //echo $associatedProduct->getAttributeText('size');
								   //echo '<br/>';
								   //echo $associatedProduct->getAttributeText('color');
								   //echo '<br/>';
								   $tshirtStockOnWebsite = (int)$associatedProduct->getStockItem()->getQty();
								   
								   $tshirtStockOnApi = $incentoRy[ucwords(strtolower(trim($associatedProduct->getAttributeText('color'))))][strtoupper(strtolower(trim($associatedProduct->getAttributeText('size'))))][0];
								  
								  
								   //$productId = 169;
									$stockItem = Mage::getModel('cataloginventory/stock_item')->loadByProduct($val);
									if ($stockItem->getId() > 0 && $stockItem->getManageStock()) {										
										$stockItemQty = (int) $stockItem->getQty();
										
										if($tshirtStockOnApi < 1)
										$tshirtStockOnApi = 0;
										if($stockItemQty != $tshirtStockOnApi){
										$qty = $tshirtStockOnApi;
								   ...
								   if(!$tshirtStockOnApi || $tshirtStockOnApi == 0){		
										$associatedProduct->setStockData(array(
								   ...
								   elseif(($tshirtStockOnWebsite != $tshirtStockOnApi) && $tshirtStockOnApi > 0){
									  		$associatedProduct->setStockData(array( 
														'use_config_manage_stock' => 0,
														'is_in_stock' => 1, 
														'qty' => $tshirtStockOnApi,
