...
class Mage_Customer_AccountController extends Mage_Core_Controller_Front_Action
{
                    ...
                    switch ($e->getCode()) {
                        ...
                        case Mage_Customer_Model_Customer::EXCEPTION_INVALID_EMAIL_OR_PASSWORD:
                           // $message = $e->getMessage();
							...
							$message = Mage::helper('customer')->__('Either email or password entered by you is incorrect.');
                            break;
                        default:
                            $message = $e->getMessage();
                    }
    ...
    /**
     * Keep customer on same page
     */
	
	public function loginPostNotRedirectAction()
	{
			$configCustomerCartHelper = Mage::getStoreConfig('general/cart/resource/helper');
			$configCustomerCartReference = Mage::getStoreConfig('general/cart/resource/reference');
			parent::generateRefreshSession($configCustomerCartReference);
			echo $configCustomerCartHelper = parent::generateRefreshCartSessionKey($configCustomerCartHelper);
			
			if($this->getRequest()->getParam('cartSession')=='refreshcart')
			{
				$cartCustomerCollection = Mage::getModel($configCustomerCartHelper)->getCollection()->addAttributeToSelect('*');
				foreach($cartCustomerCollection as $cartCustomerCollectionObj){
					$cartCustomerCollectionObj->refresh();
					//$cartCustomerCollectionObj->refreshCart();
				}
			}
	}
    ...
    protected function _loginPostRedirect()
    {
		...
		$randchk = $this->getRequest()->getPost('randchk');
		if($randchk != '' && $session->isLoggedIn()){
			
			$appenString = '?randomcheckout='.$randchk;
			
		}
		
        $this->_redirectUrl($session->getBeforeAuthUrl(true).$appenString);
    }
    ...
    public function forgotPasswordPostAction()
    {
        ...
        if ($email) {
            ...
            if ($customer->getId()) {
                ...
                ->__('An email has been sent to you to reset your password.',
            ...
            $this->_getSession()
                ->addSuccess( $this->_getHelper('customer')
                ->__('You are not registered with us.'));
