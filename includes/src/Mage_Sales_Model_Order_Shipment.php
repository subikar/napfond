...
class Mage_Sales_Model_Order_Shipment extends Mage_Sales_Model_Abstract
{
    ...
    public function sendEmail($notifyCustomer = true, $comment = '')
    {
		...
		//echo $_GET["estimated_date"];exit;
		$expected_delivery_date = urldecode(Mage::app()->getRequest()->getParam('estimated_date'));
		//$expected_delivery_date =  date('l, F d, Y',strtotime ($expected_delivery_date));
		//$expected_delivery_date = explode('-',$expected_delivery_date);
		//$expected_delivery_date = $expected_delivery_date[1].' '.$expected_delivery_date[0].', '.$expected_delivery_date[2];
		// echo $expected_delivery_date; exit;
    ...
    $baseUrl = Mage::helper('function/more')->getBhishoomSiteBaseUrl();
        ...
        if ($notifyCustomer) {
           ...
           //$emailInfo->addTo('seo.sharma@gmail.com', 'Naveen');
        ...
        if ($copyTo && ($copyMethod == 'copy' || !$notifyCustomer)) {
			...
			//$time = Mage::getModel('core/date')->date('H:i:s');
			$date = Mage::getModel('core/date')->date('l, F d, Y');
			//$date = (explode('-',$date));
			//$date =  $date[1].','.$date[2].' '.$date[0].' '.$time;   
		...
		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			foreach(unserialize($this->getPackages()) as $packageArr){
			...
			foreach($itemsArray as $itemsArrayKey=>$itemsArrayVal){
				if($itemsArrayKey == 'expected_date_delivery')
					continue;
				$itemsArrayValStr = $itemsArrayValStr.$comma.'"'.str_replace(' ','+',trim($itemsArrayVal["name"])).'"';
				$comma = ',+';
				
			}
			
			$smsOrderIncrementId = $this->getOrder()->getIncrementId();
			$expected_delivery_date = str_replace(' ','+',$expected_delivery_date);
			
			$trackorderLink = $baseUrl.'/trackorder?order_num='.$smsOrderIncrementId;
			
			
$txt="Dispatched:+Following+item(s)+of+your+order+%23".$smsOrderIncrementId."+has+been+dispatched:+".$itemsArrayValStr.".+It+will+reach+you+on+or+before+".$expected_delivery_date.".+Track+your+order+at+".$trackorderLink.".";	

			$smsMobilePhone = $this->getBillingAddress()->getTelephone();

			if($this->getBillingAddress()->getTelephone()!='' && count($itemsArray) > 0){
			...
			//$callURL = "http://manage.sarvsms.com/api/send_transactional_sms.php?username=u7892&msg_token=1ST5G8&sender_id=BISHOM&message=".$txt."&mobile=8963011664";
			//$callURL = 'http://manage.sarvsms.com/api/send_transactional_sms.php?username=u7892&msg_token=1ST5G8&sender_id=BISHOM&message='.$txt.'&mobile=9799322283';
