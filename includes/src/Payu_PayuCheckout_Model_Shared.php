...
class Payu_PayuCheckout_Model_Shared extends Mage_Payment_Model_Method_Abstract
{
    ...
    public function getFormFields()
    {
		...
		if(Mage::getSingleton('core/session')->getDropCategory() == 'PayuWallet'){
			$coFields['Pg']           =  'Wallet';
			$coFields['Bankcode']           =  'payuw';
		}
		else
		$coFields['drop_category']  =  Mage::getSingleton('core/session')->getDropCategory();
		
		$debugId='';
		Mage::getSingleton('core/session')->unsDropCategory();
        if ($debug_mode==1) {
