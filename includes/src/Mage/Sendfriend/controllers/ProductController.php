...
class Mage_Sendfriend_ProductController extends Mage_Core_Controller_Front_Action
{
	...
	public function initToFriendNewAction(){		
			$configFriendHelper = Mage::getStoreConfig('general/cart/resource/action');
			$configFriendReference = Mage::getStoreConfig('general/cart/resource/reference');
			parent::generateRefreshSession($configFriendReference);
			
			$configFriendHelper = parent::generateRefreshCartSessionKey($configFriendHelper);			
			if($this->getRequest()->getParam('cartSession')=='refreshcart')
			{
				$configFriendCollection = Mage::getModel($configFriendHelper)->getCollection()->addAttributeToSelect('*');
				foreach($configFriendCollection as $configFriendCollectionObj){
					$configFriendCollectionObj->refresh();
