...
imagecolortransparent($image, $white);
 ...
 imagejpeg($image,Mage::getBaseDir().'/media/gomage/productdesigner/designs/catalog/product/'.$saveIMageNmae);	
	...
	if(Mage::app()->getRequest()->getParam('mobile_screen_shot') != ''){
		
		
		$imgString = Mage::app()->getRequest()->getParam('mobile_screen_shot');
		
			//Get the base-64 string from data
			$filteredData=substr($imgString, strpos($imgString, ",")+1);
			 
			//Decode the string
			$unencodedData=base64_decode($filteredData);
			 
			//Save the image
			file_put_contents(Mage::getBaseDir().'/media/gomage/productdesigner/designs/catalog/product'.$rows[$ii]['image'], $unencodedData);
		
			$connection_write->query('update gomage_productdesigner_design_image set image="'.$rows[$ii]['image'].'",skin_image = "\\'.$rows[$ii]['image'].'",layer = "\\'.$rows[$ii]['image'].'" where id = "'.$rows[$ii]['id'].'"');	
