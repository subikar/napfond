...
class ES_Newssubscribers_Model_Subscriber extends Mage_Newsletter_Model_Subscriber
{
    ...
    public function getCouponPromotionCode()
    {
	     //echo 'hellooo and that';exit;
        ...
        $model = Mage::getModel('salesrule/rule');
        $model->load(Mage::getStoreConfig('newsletter/coupon2/roleid'));
        $massGenerator = $model->getCouponMassGenerator();
        $session = Mage::getSingleton('core/session');
        $ruleId = Mage::getStoreConfig('newsletter/coupon2/roleid');
        if (!is_numeric($ruleId)) {
            return self::ERROR_SHOPPING_CARD_RULE_IS_MISSING;
        }
        $rule = Mage::getModel('salesrule/rule')->load($ruleId);
        ...
        try {
            $massGenerator->setData(array(
                'rule_id' => $ruleId,
                'qty' => 1,
                'length' => Mage::getStoreConfig('newsletter/coupon2/length'),
                'format' => Mage::getStoreConfig('newsletter/coupon2/format'),
                'prefix' => Mage::getStoreConfig('newsletter/coupon2/prefix'),
                'suffix' => Mage::getStoreConfig('newsletter/coupon2/suffix'),
                'dash' => Mage::getStoreConfig('newsletter/coupon2/dash'),
                'uses_per_coupon' => 1,
                'uses_per_customer' => 1
            ));
            $massGenerator->generatePool();
