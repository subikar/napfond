...
GoMage.TextEditor.prototype = {
    ...
    selectColorOnPicker: function(e) {
        ...
        if (obj && obj.type == 'custom_text') {
            ...
            var obj = this.productDesigner.canvas.getActiveObject();
			//obj.text = jQuery("#add_text_textarea").val();
			
			
			obj.stroke = e.hex;
			//obj.fontSize = jQuery("#font_size_selector").val();
			 
			//alert(obj.shadow)
			
			//obj.setShadow(color+' 23px 23px 23px');
    ...
    observeFontChange: function(){  
        ...
        this.fontSelector.observe('change', function(e) {
            ...
            var obj = this.productDesigner.canvas.getActiveObject();
			//obj.text = jQuery("#add_text_textarea").val();
			obj.fontFamily = jQuery("#font-selector").val();
			//obj.fontSize = jQuery("#font_size_selector").val();
    ...
    observeAddText: function(){
        ...
        this.addTextTextarea.observe('keyup', function(e) {
			...
			if (!obj)
				return;
			if (obj.type != 'text' && obj.type != 'custom_text')
				return;
    ...
    observeFontStyleControls: function(){
        ...
        this.addTextBtnItalic.observe('click', function(e) {
			...
			//obj.text = jQuery("#add_text_textarea").val();
			//obj.fontFamily = jQuery("#font-selector").val();
			//obj.fontStyle = {fontStyle: (!obj.fontStyle ? 'italic' : '')}
        ...
        this.addTextBtnUnderline.observe('click', function(e) {		
			...
			//obj.text = jQuery("#add_text_textarea").val();
			//obj.fontFamily = jQuery("#font-selector").val();
			//obj.textDecoration = {textDecoration: (!obj.textDecoration ? 'underline' : '')}			
			
			if(obj.textDecoration == '')
				obj.textDecoration = 'underline'
			else if(obj.textDecoration == 'underline')	
				obj.textDecoration = ''			
    ...
    observeShadowButton: function(){
        ...
        this.btnShadowText.observe('click', function(e){
            ...
            if (obj && obj.type == 'custom_text') {
				...
				if(typeof obj.textShadow=='undefined'){
					obj.textShadow = ''
				}
                if (obj.textShadow == '') {
                    this.setShadow({
    ...
    setShadow: function(shadowParams) {
        ...
        if (obj && obj.type == 'custom_text') {
			...
			blankpara = '';
			shadowval = ''
			if(typeof shadowParams['textShadowColor'] != 'undefined')
			 {
			   shadowval = shadowval + blankpara + shadowParams['textShadowColor'];			   
			   blankpara = ' ';
			 }
			else{
			   shadowval = shadowval + blankpara + '#000000';			   
			   blankpara = ' ';			
			}

			
			 shadowval = shadowval + blankpara +  jQuery("#shadow_x_range").val()+'px '+ jQuery("#shadow_y_range").val()+'px '+jQuery("#shadow_blur").val()+'px';
			 
			obj.setShadow(shadowval);
			objCallDirectForCanvas.renderAll();
			objCallDirectForCanvas.calcOffset();
			return;	

            var cmd = new TransformCommand(this.productDesigner.canvas, obj, {shadow : shadowParams});
            cmd.exec();
    ...
    observeOutlineButton: function(){
        ...
        this.btnOutlineText.observe('click', function(e){
			...
			this.toggleConfigContainer(elem);	
				...
				//alert(this.defaultFieldsValues.strokeStyle)
			...
			obj.strokeWidth = this.defaultFieldsValues.strokeWidth;
			//obj.fontSize = jQuery("#font_size_selector").val();
    ...
    observeOutlineControls: function(){
        ...
        this.outlineStrokeWidthRange.observe('change', function(e) {
			...
			//alert(elem.value)
			obj.strokeWidth = eval(elem.value);
			//obj.fontSize = jQuery("#font_size_selector").val();
...
GoMage.ImageUploader.prototype = {
        ...
        $('uploadImages').onsubmit = function(){
			if ($('filesToUpload')) {
                ...
                $('iframeSave').onload = function() {
					...
					jQuery('#social_media_img_container').append('<li><img class="clipart-image tplcustUpload_Image" src="'+gra_new_url+'" data-origin-url="'+new_url+'"><span onclick="jQuery(this).parent().remove();">X</span></li>')	
					
					
					this.show_popup_crop(currentUploadedDir,currentUploadedDir,baseNameTplCust(new_url),curr_obj_id,objsImage[curr_obj_id].width,objsImage[curr_obj_id].height,'upload');
					
					/*
					
	imgname = baseNameTplCust(new_url);
	url = currentUploadedDir;
	basepath = currentUploadedDir;
	imgname = baseNameTplCust(new_url)
	wdh = objsImage[curr_obj_id].width;
	hgt = objsImage[curr_obj_id].height;
	...
	show_popup_crop: function(url,basepath,imgname,popupid,wdh,hgt,type) {
	if(type=='upload')
	url = baseUrlForMedia+'gomage/productdesigner/uploadedImage/'+url+'/'+imgname;	
	else if(type=='stock'){ 
			url = baseUrlForMedia+'stock/img-600/'+imgname;	
	}
	else if(type=='clipart'){
	imgArr = imgname.split();	
	url = baseUrlForMedia+'gomage/productdesigner/cliparts/'+imgArr[0]+'/'+imgArr[1]+'/'+imgname;	
	}
	else if(type=='facebook')
	url = baseUrlForMedia+'gomage/productdesigner/cliparts/'+imgname;	
	...
	jQuery('#photo').val('');
	jQuery('#popup_id').val(popupid);
	jQuery('#photo_url_type').val(type);
	// show the crop popup
	...
	var testElement = objCallDirectForCanvas;
	var photo_url_type = jQuery('#photo_url_type').val();
	...
	jQuery.ajax({
		...
		type: 'POST',
		data: {x:x_, y:y_, w:w_, h:h_, photo_url:photo_url_, targ_w:TARGET_W, targ_h:TARGET_H,UploadedDir:currentUploadedDir,photo_url_type:photo_url_type},
		success:function(data){ 
		   ...
		   //jQuery('#social_media_img_container').append('<li><img class="clipart-image" src="'+data+'" data-origin-url="'+data+'"><span onclick="jQuery(this).parent().remove();">X</span></li>');
					...
					//var tmp=objsImage[curr_obj_id].getElement();
					 
					objsImage[curr_obj_id].setSrc(data,function(){
						 
						objCallDirectForCanvas.renderAll();
						objCallDirectForCanvas.calcOffset();
					
					}); 
					//alert(tmp.getAttribute("src"))
    ...
    observeImageSelect: function(){
        Event.on($('uploadedImages'), 'click', '.clipart-image', function(e, elm){
			...
			arrSplit = img.getAttribute('data-origin-url').split('/');
			//currentUploadedDir = arrSplit[arrSplit.length-2]
			
			this.show_popup_crop(currentUploadedDir,currentUploadedDir,arrSplit[arrSplit.length-2]+'/'+baseNameTplCust(img.getAttribute('data-origin-url')),curr_obj_id,objsImage[curr_obj_id].width,objsImage[curr_obj_id].height,'stock');
	...
	observeImageSelect3: function(){
        Event.on($('social_media_img_container'), 'click', '.clipart-image', function(e, elm){
			...
			arrSplit = img.getAttribute('data-origin-url').split('/');
			arrSplitClass = img.getAttribute('class').split(' ');
			//currentUploadedDir = arrSplit[arrSplit.length-2]
			
			
			
			if(arrSplitClass.indexOf("tplcustUpload_Image") > -1){
				imgGetFrom = 'upload';
				imggname = baseNameTplCust(img.getAttribute('data-origin-url'))
			}	
			else if(arrSplitClass.indexOf("tplcustStock_Image") > -1){
				imgGetFrom = 'stock';
				imggname = arrSplit[arrSplit.length-2]+'/'+baseNameTplCust(img.getAttribute('data-origin-url'))
			}	
			
			
			this.show_popup_crop(currentUploadedDir,currentUploadedDir,imggname,curr_obj_id,objsImage[curr_obj_id].width,objsImage[curr_obj_id].height,imgGetFrom);
