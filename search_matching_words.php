...
foreach($options as $option){
			...
			if($id != ''){
				...
				//$match_cat_child_id = $category->getId();
				echo $query = "insert into a_search_matching_words set match_word='".trim($option["label"])."',match_word_type='style',match_word_db_id='".$option["value"]."'";
				$write->query($query);				
			}
...
function getChildCategories($catId,$level=''){
...
global $write;
	...
	if($children->count() > 0)
	{
		foreach ($children as $category) {
			...
			//echo "-->";
			//echo $category->getIsActive();
			//echo "<br/>";
				
			getsubCats($catId,$category);
			getChildCategories($category->getId(),$category->getLevel());
		}
	}else{		
...
function getsubCats($catId,$subCat)
{
